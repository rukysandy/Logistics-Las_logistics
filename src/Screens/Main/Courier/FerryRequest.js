...
export default function FerryRequest({ navigation }) {
    const [ferryOrders, setFerryOrders] = useState([])

    
    const getFerryOrders = async () => {
        const events = await firebase.firestore().collection('Orders').where("carrierType", "==", "FERRY")
        events.get().then((querySnapshot) => {
            const tempDoc = []
            querySnapshot.forEach((doc) => {
                tempDoc.push({ id: doc.id, ...doc.data()})
            })
            setFerryOrders(tempDoc)
            console.log(tempDoc)
        })
    }
