...
export default function AirRequest({ navigation }) {
    const [airOrders, setAirOrders] = useState([])


    const getAirOrders = async () => {
        const events = await firebase.firestore().collection('Orders').where("carrierType", "==", "AIR")
        events.get().then((querySnapshot) => {
            const tempDoc = []
            querySnapshot.forEach((doc) => {
                tempDoc.push({ id: doc.id, ...doc.data() })
            })
            setAirOrders(tempDoc)
            console.log(tempDoc)
        })
    }
