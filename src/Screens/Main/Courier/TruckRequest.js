...
export default function TruckRequest({navigation}) {
    ...
    const getTruckOrders = async () => {
        const events = await firebase.firestore().collection('Orders').where("carrierType", "==", "TRUCK")
        events.get().then((querySnapshot) => {
            const tempDoc = []
            querySnapshot.forEach((doc) => {
                tempDoc.push({ id: doc.id, ...doc.data() })
            })
            setTruckOrders(tempDoc)
            console.log(tempDoc)
        })
    }
    ...
    return (
        ...
        <View style={styles.modalCard}>
            {truckOrders.map((item, i) => {
                return (
                    <TouchableOpacity
                        key={i}
                        onPress={() => {
                            navigation.navigate('MyRequestDetails',{ orderParameters:item })
                        }}
